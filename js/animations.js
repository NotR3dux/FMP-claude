...
function makeStar(colors) {
  ...
  const del   = (Math.random() * 2.5).toFixed(2);
  const sz    = (Math.random() * 0.7 + 0.5).toFixed(2);
  path.setAttribute('d', SPARKLE_PATH);
  path.setAttribute('fill', color);
  svg.appendChild(path);
  svg.setAttribute('class', 'sparkle-star');
  ...
  svg.style.setProperty('--del', del + 's');
  svg.style.setProperty('--sz', sz);
  return svg;
}

export function initSparkles() {
  document.querySelectorAll('.sparkle-text').forEach(el => {
    const colors = (el.dataset.sparkleColors || '#FAC740,#E8684A').split(',');
    const count  = parseInt(el.dataset.sparkleCount || '8');
    const wrap   = document.createElement('span');
    wrap.className = 'sparkle-wrap';
    wrap.innerHTML = el.innerHTML;
    el.innerHTML   = '';
    el.appendChild(wrap);
    for (let i = 0; i < count; i++) wrap.appendChild(makeStar(colors));
    // refresh sparkles periodically so they feel alive
    setInterval(() => {
      const old = wrap.querySelectorAll('.sparkle-star');
      if (old.length) old[Math.floor(Math.random() * old.length)].replaceWith(makeStar(colors));
    }, 900);
  });
}
